...
fn main() {
    ...
    let mut seg = SegTree::<usize>::new(&vec![0; n + 1]);
    seg.set(0, 1);
    seg.set(1, P - 1);
    for i in 0..n {
        let val = seg.prod(0, i + 1);
        for (l, r) in &lr {
            if i + l >= n {
                continue;
            }
            let vl = seg.prod(i + l, i + l + 1);
            seg.set(i + l, (vl + val) % P);

            let indr = (i + r + 1).min(n);
            let vr = seg.prod(indr, indr + 1);
            seg.set(indr, (P + vr - val) % P);
        }
    }
    println!("{}", seg.prod(0, n));
}
